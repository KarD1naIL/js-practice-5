...
function printUppercase(array) {
  for (const stil of array) {
    console.log(stil.toUpperCase());
  }
...
function concatenateStrings(array, subString) {
  const conca = array.toString();
  const catena = conca.includes(subString);
  console.log(catena);
...
function replaceNumbers(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i] > 10) {
      ...
      replace.push("Less than or equal to 10");
    }
  }
  return replace;
